<div class="slider-admin-tools"><h2><?php echo $this->lang('My sliders') ?> <span class="slider-count"><?php echo count($sliders) ?></span></h2><?php if ($this->access('slider', 'manage_sliders')) echo $this->button_create('admin/slider/add', (string)$this->lang('Create a slider')); ?></div>
...
<div class="slider-set-meta"><span><?php echo count($row['slides']) ?> slide<?php echo count($row['slides']) > 1 ? 's' : '' ?></span><span><?php echo utf8_htmlentities(\HiddenCMS\Slider\Settings::effects()[$row['effect']]) ?></span><span><?php echo (int)$row['height'] ?> px</span><span><?php echo $row['autoplay'] ? (string)$this->lang('Automatic') : 'Manuel' ?></span></div>
...
<?php if (!$sliders): ?><div class="slider-empty"><?php echo icon('far fa-images') ?><p><?php echo $this->lang('No sliders') ?></p></div><?php endif ?>
</div>
